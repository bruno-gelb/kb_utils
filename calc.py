...
def calc_damage_modifier(attackers_attack: int, defenders_defend: int) -> float:
...
def versus(your: Unit, target: Unit) -> None:
    ...
    modifier = calc_damage_modifier(your.attack, target.defend)
    ...
    if modifier < MAX_MODIFIER:
        ...
        click.echo(f'{modifier} > {MAX_MODIFIER}, so your attack is suboptimally high!')
        # todo add an explanation how much high exactly

    click.echo(LINE)
    click.echo(f'{target} attacking {your}: {target.attack} >> {your.defend}')
    modifier = calc_damage_modifier(target.attack, your.defend)
    click.echo(f'modifier is {modifier}')
    if modifier > MIN_MODIFIER:
